        ...
        >
          <div class="p-4 md:p-5">
            ...
            <div class="mt-10 grid lg:grid-cols-2 gap-6">
              ...
              <div>
                <button id="submitData" name="submitData" v-on:click="SubmitDataCalegDprd1" class="font-bold bg-gradient-to-r from-yellow-400 to-yellow-300 w-44 hover:bg-green-950 text-white p-2 border border-1 rounded-xl mt-10 border-slate-900">Kirim Data</button>
              </div>
            </div>
          </div>
        </div>
  ...
  queryTimeOut.value = setTimeout(async () => {
    try {
      const ResultKota = await axios.post(
        `http://localhost:8080/inputcalegdprd1`,
        formValues,
        {
          Headers: {
