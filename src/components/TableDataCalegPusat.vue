...
const cariDataCalegPusat=(kode_propinsi)=>{
    
    clearTimeout(queryTimeOut.value);
  queryTimeOut.value = setTimeout(async () => {
    try {
      const ResultCalegPropinsi = await axios.get(
        `http://localhost:8080/calegpusat/${kode_propinsi}`
      );
