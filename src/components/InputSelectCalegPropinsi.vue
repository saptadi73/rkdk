<template>
  <div>
    ...
    >
      <p
        @click="IsiInput(resultKu.nama, resultKu.id)"
    ...
    <div>
      <input type="text" id="idpropinsi" name="idpropinsi" />
    </div>
  </div>
</template>
...
const IsiInput = (searchQuery, searchID) => {
  ...
  document.getElementById("idpropinsi").value = searchID;
  document.getElementById("listPropinsi").classList.add("hidden");
  document.getElementById("idpropinsi").classList.add("hidden");
};
