                ...
                <div class="flex flex-col bg-gradient-to-r from-slate-200 to-slate-100 border shadow-sm rounded-lg">
                    <div class="p-4 md:p-5">
                        <h3 class="text-lg font-bold text-black dark:text-white">
                            Pengisian Partai Tingkat Kota/Kabupaten
                        </h3>
                        <div class="mt-10 grid lg:grid-cols-2 gap-6">
                            ...
                            <div>
                                <label for="kota" class="text-gray-800 text-sm font-medium block mb-2">Pilih Kota atau
                                    Kabupaten</label>
                                <input id="kota" name="kota"
                                    class="form-input py-3 px-4 block w-full border-gray-200 rounded-md text-sm focus:border-blue-500 focus:ring-blue-500" />
                                <div id="listKota" name="listKota" v-if="hasilku"
                                    class="w-[300px] h-[200px] overflow-scroll">
                                    <p @click="IsiInputKota(resultKu.nama, resultKu.kode)" v-for="resultKu in hasilku.data"
                                        :key="resultKu.id"
                                        class="p-2 text-black font-semibold cursor-pointer hover:text-slate-500 bg-gradient-to-r from-yellow-200 to-white">
                                        {{ resultKu.nama }}
                                    </p>
                            ...
                            <input type="hidden" id="id_partaikota" name="id_partaikota" />
                            <div>
        ...
        <div id="segment-2" class="hidden" role="tabpanel" aria-labelledby="segment-item-2">
            <div class="flex flex-col gap-6">
                <div class="flex flex-col bg-gradient-to-r from-slate-200 to-slate-100 border shadow-sm rounded-lg">
                    <div class="p-4 md:p-5">
                        <h3 class="text-lg font-bold text-black dark:text-white">
                            Data Pengurus Partai Tingkat Kota/Kabupaten
                        </h3>
                        <div class="mt-10 grid lg:grid-cols-2 gap-6">
                            <div>
                            ...
                            <button id="simpan_data"
                                ...
                                v-on:click="SubmitUpdatePartaiKota">
                                Update Data
                            </button>
                        </div>
                    </div>
...
export default {
    ...
    methods: {
        ...
        async PreviewKota(PropinsiID) {
            await axios.get(`https://wilayah.rimang.id/kota/${PropinsiID}`).then((response) => {
        ...
        async UpdatePartaiKota() {
            this.id = this.$route.params.id;

            await axios
                .get(`http://localhost:8080/findpartaikota/${this.id}`, {
                    Headers: {
                ...
                .then((response) => {
                    ...
                    document.getElementById(
                        "id_partaikota"
                    ).value = this.hasilupdate.data.id_partaikota;
        ...
        async SubmitUpdatePartaiKota() {
            if (this.hasilupdate.data.id_registrasi == this.cookies.get("id")) {
                ...
                const m_id_partaikota = document.getElementById("id_partaikota").value;
                ...
                await axios
                    .post(`http://localhost:8080/updatepartaikota`, formValues, {
                        Headers: {
                            "Content-Type": "application/x-www-form-urlencoded",
                        },
                    })
